const { getBalanceUrl, getNormalTransactionUrl, txHash}= require('./queryParams');
    ...
    async function getAllTransactionsControllers(req, res) {
        ...
        try {
          while (true) {
            ...
            sendSSE({ type: 'progress', message: `Fetching batch ${batchCount + 1}...` });
      
            const transactions = await getNormalTransactions(address, lastSeenTxId, BATCH_SIZE);
            ...
            filteredTransactions.sort((a, b) => parseInt(b.value) - parseInt(a.value));
            ...
            if (remainingDelay > 0) {
              sendSSE({ type: 'info', message: `Waiting for ${remainingDelay}ms before next batch...` });
      ...
      const getNormalTransactions = async (address, lastSeenTxId = null, limit = 50) => {
        try {
            let url;
            if (lastSeenTxId) {
...
module.exports = { getBalance, getNormalTransactions, getAllTransactionsControllers };